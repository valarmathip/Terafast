...
def editConfig_createPortLabel(prtIds):
    ...
    if str(prtIdSymbol[1]) == ',' or str(prtIdSymbol[1]) == '':
...
def cmpIngEgrPortLst(xmlData):
    ...
    logger.info('port Id list: %s' % xmlPortIds)
    logger.info('Port Label Names list: %s\n\n' % xmlPrtLblNames)

    
    if cmp(prtIdLst, xmlprtIdLst) == cmp(PrtLblNames, xmlPrtLblNames):
...
def editConfig(tcName, xmlstr):
    ...
    try:
       ...
       e = time.time()
       t = int(round((e - s)* 1000))
       csvOutput('Ports', tcName, t, result)

    except Exception as err:
       print '\n\n'
       logger.error('\t\t-----[ Error from the Switch ]-----\n\n%s\n\n', err)
       csvOutput('Ports', tcName, 0, result)
